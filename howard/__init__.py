...
def from_dict(d: dict, t: Generic[T], decoders=None, ignore_extras: bool=True) -> T:
    """
    ...
    return _convert_to(d, t, decoders if decoders else {}, ignore_extras)
...
def _convert_to(obj, t, decoders, ignore_extras=True):
    kwargs = {}
    if dataclasses.is_dataclass(t):
        for f in dataclasses.fields(t):
            if f.name in obj:
                # get value
                value = obj[f.name]
                kwargs[f.name] = _convert_to(value, f.type, decoders, ignore_extras=ignore_extras)
        ...
        if real_type == list:
            return [
                _convert_to(i, args[0], decoders, ignore_extras=ignore_extras)
                for i in obj
            ]
        elif real_type == dict:
            return {
                _convert_to(k, args[0], decoders, ignore_extras=ignore_extras): _convert_to(v, args[1], decoders, ignore_extras=ignore_extras)
